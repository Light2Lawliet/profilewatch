...
detection, and Claude-powered auto-remediation — instead of the usual
...
3. **Auto-remediates with Claude**: for every open incident, asks Claude for
...
4. **Prescribes a success plan for any account**, not just ones on fire:
   Claude ranks the highest-leverage priorities — whether that's fixing an
...
5. **Writes a weekly digest**: one more Claude call takes every account's
...
6. **Flags churn risk**: if an account's health score has declined for two
   ...
   corroborating it, not just a bare streak — it's flagged, and Claude drafts
...
the specific listing fields that disagree — no Claude call required just to
...
| **Root-cause analysis** | On-call engineer reads logs/traces | Claude reads the account's reviews/listings and explains what's actually wrong |
| **Auto-remediation** | Auto-restart, auto-scale, runbook execution | Claude drafts the review reply and names the concrete fix action |
...
| Drafting a reply to a negative review | ~10 min (when it happens) | Claude drafts it; owner reviews and posts |
...
- **AI**: Anthropic Claude via the Messages API, using
  `output_config.format` (JSON schema) for reliably parseable structured
  output on every Claude call
...
file? Exporting `ANTHROPIC_API_KEY` in the shell before running still works exactly
as before.
...
- "Explain & draft response", "Generate save play", and "Generate Weekly
  Digest" all call Claude and require `ANTHROPIC_API_KEY`. If it's missing,
...
|---|---|---|
| `ANTHROPIC_API_KEY` | — | Required for all Claude-powered features |
| `ANTHROPIC_MODEL` | `claude-sonnet-4-6` | Model used for every Claude call |
...
2. **Render** → New → Blueprint → select the repo. Set `ANTHROPIC_API_KEY` and `PROFILEWATCH_ACCESS_KEY` (any long random string).
...
The UI calls `https://profilewatch-api.onrender.com` when it isn't on localhost. If your Render service has a different name, update `API_BASE` in `src/profilewatch.html`. When `PROFILEWATCH_ACCESS_KEY` is set, the UI asks for the key once, on the first AI button click, and stores it in the browser. AI calls are also rate-limited per IP (`PROFILEWATCH_CLAUDE_RATE_LIMIT_PER_HOUR`, default 30). Render's free tier sleeps when idle, so the first load after a pause can take about 50 seconds.

## API

| Endpoint | Method | Description |
|---|---|---|
| `/api/status` | GET | Whether Claude is configured, current model, anchor date, health-score formula version |
| `/api/accounts` | GET | All accounts with computed health score (+ per-signal `score_components` breakdown), incidents, opportunities, churn risk — zero Claude calls |
| `/api/accounts/{id}` | GET | Single account detail |
| `/api/accounts/{id}/explain` | POST | Claude: root cause, suggested review reply, and a 2–4 step `action_plan` (400 if no open incidents) |
| `/api/accounts/{id}/save-play` | POST | Claude: CSM save play (400 if not flagged churn-risk) |
| `/api/accounts/{id}/success-plan` | POST | Claude: ranked, prescriptive priorities for *any* account — no incident/churn-risk gate |
| `/api/digest` | POST | Claude: one paragraph + one top priority per account for a weekly owner email |
